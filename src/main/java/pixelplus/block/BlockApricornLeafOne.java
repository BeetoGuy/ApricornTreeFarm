...
public class BlockApricornLeafOne extends BlockLeaves
{
    ...
    public BlockApricornLeafOne()
    {
        ...
        this.setUnlocalizedName("pixelplus:apricorn_leaf_first");
    }
        ...
        if(!world.isRemote)
        {
            ...
            if(below.isAir(world.getBlockState(pos.down()), world, pos) || below.isReplaceable(world, pos.down()))
            {
                if(rand.nextInt(30) == 0) {
                    world.setBlockState(pos.down(), PixelRegistry.apricornOne.getDefaultState().withProperty(BlockApricornComplex.APRICORNS, state.getValue(APRICORNS)));
