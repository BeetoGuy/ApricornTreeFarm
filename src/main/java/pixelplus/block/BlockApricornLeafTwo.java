...
public class BlockApricornLeafTwo extends BlockLeaves
{
    ...
    public BlockApricornLeafTwo()
    {
        ...
        this.setUnlocalizedName("pixelplus:apricorn_leaf_second");
    }
        ...
        if(!world.isRemote)
        {
            ...
            if(below.isAir(world.getBlockState(pos.down()), world, pos) || below.isReplaceable(world, pos.down()))
            {
                ...
                if(rand.nextInt(chance) == 0) {
                    world.setBlockState(pos.down(), PixelRegistry.apricornTwo.getDefaultState().withProperty(BlockApricornPrimary.APRICORNS, state.getValue(APRICORNS)));
                ...
                else if(canHybridize(world, pos, state))
                {
                    if(rand.nextInt(802) == 0) {
                        world.setBlockState(pos.down(), PixelRegistry.apricornTwo.getDefaultState().withProperty(BlockApricornPrimary.APRICORNS, EnumApricorns.PURPLE));
                        if (!state.getValue(DECAYABLE))
                            world.setBlockState(pos, state.withProperty(DECAYABLE, true));
                    }
                }
    ...
    {
        if (!PixConfig.masterBallCrafting || !PixConfig.driversExist) return false;
    ...
    {
        return Item.getItemFromBlock(PixelRegistry.apricornSapling);
    }
